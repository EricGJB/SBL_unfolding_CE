...
optimized_phases_F = weight_dict['F_phases']
optimized_phases_W = weight_dict['W_phases']

W = (np.cos(optimized_phases_W)+1j*np.sin(optimized_phases_W))/np.sqrt(Nr)
W = np.matrix(W)
F = (np.cos(optimized_phases_F)+1j*np.sin(optimized_phases_F))/np.sqrt(Nt)
F = np.matrix(F)
...
# a_list = [0.2,0.4,0.6,0.8,1,1.2,1.4]
...
file_handle.write('SBL with optimized W and F, SNR=%d dB:\n'%SNR)
