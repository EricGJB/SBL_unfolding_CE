...
from functions import dictionary,Phi_Layer_joint_opt,update_mu_Sigma,circular_padding_2d,A_R_Layer,A_T_Layer
...
def SBL_net(Mt, Mr, Nt, Nr, G, N_r_RF, num_sc, num_layers, num_filters, kernel_size):
    ...
    Phi_real_imag, y_real_imag = Phi_Layer_joint_opt(Nt, Nr, Mt, Mr, G, N_r_RF, num_sc)([H_real_imag,noise_real_imag])
...
batch_size = 16
best_model_path = './models/best_Mr_%d_Mt_%d_SNR_%d_joint_opt.h5'%(Mr,Mt,SNR)
...
init_phases_F = np.random.uniform(0, 2 * np.pi, Nt*Mt)
init_phases_F = np.reshape(init_phases_F,(Nt,Mt))

weight_dict = np.load('./results/weight_dict_Mr_%d_Mt_%d_SNR_%d_random_init.npy'%(Mr,Mt,SNR),allow_pickle=True).item()
...
file_handle.write('Joint opt, single SNR training, SNR=%d dB:\n'%SNR)
...
np.save('./results/weight_dict_Mr_%d_Mt_%d_SNR_%d_joint_opt.npy'%(Mr,Mt,SNR),weight_dict)
print('Weight dict saved!')


# save the single block results for the multi-block training
H_list = io.loadmat('./data/channel_previous.mat')['H_list'][-data_num:]
noise_list = np.sqrt(sigma_2/2)*\
    (np.random.randn(data_num,Mr//N_r_RF,Nr,Mt*num_sc)\
     +1j*np.random.randn(data_num,Mr//N_r_RF,Nr,Mt*num_sc))
noise_list = np.expand_dims(noise_list, axis=-1)
...
# notice that the order of X is correct, i.e., AoA before AoD
for i in range(len(model.layers)):
    if model.layers[i].name=='X_hat':
        X_hat_index = i
get_X_hat = K.function([model.input],[model.layers[X_hat_index].output])
X_real_imag_previous_block = np.zeros((data_num,G,G,num_sc,2))
for i in range(data_num//25): # multiple samples each time
    if i%40 == 0: # print every 1000 samples
        print('Fetching X_hat of sample %d'%(25*i))
    X_real_imag_previous_block[25*i:25*(i+1)] = get_X_hat([H_list[25*i:25*(i+1)],\
                                        noise_list[25*i:25*(i+1)]])[0]
X_abs_previous_block = np.abs(X_real_imag_previous_block[:,:,:,:,0]+1j*X_real_imag_previous_block[:,:,:,:,1])
io.savemat('./results/X_abs_previous_block_Mr_%d_Mt_%d_SNR_%d.mat'%(Mr,Mt,SNR),{'X_abs_previous_block':X_abs_previous_block})
print(X_abs_previous_block.shape)
