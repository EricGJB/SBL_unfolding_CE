...
normalized_AoAs_list = np.zeros((data_num,Lp))
normalized_AoDs_list = np.zeros((data_num,Lp))
...
for i in range(data_num):
    ...
    normalized_AoAs_list[i] = normalized_AoAs
    normalized_AoDs_list[i] = normalized_AoDs
...
io.savemat('./data/channel_previous.mat',{'H_list':H_list})
print(H_list.shape)  # (data_num,Nr,Nt,num_sc,2)

del H_list


#%% generate channel of the current block
# rou = 0.92245
# rou = np.sqrt(0.5)
rou = 0.916

# AR model for the path gains
noise_list = np.sqrt(1/2)*(np.random.randn(data_num,Lp)+1j*np.random.randn(data_num,Lp))
Gains_list = np.sqrt(1 - rou ** 2) * noise_list + rou * Gains_list
...
for i in range(data_num):
    if i % 500 == 0:
        print('%d/%d' % (i, data_num))
    normalized_AoAs = normalized_AoAs_list[i]
    normalized_AoDs = normalized_AoDs_list[i]
    for n in range(num_sc):
        f = n * eta
        H_subcarrier = np.zeros((Nr, Nt)) + 1j * np.zeros((Nr, Nt))
        for l in range(Lp):
...
# put the dimension num_sc after Nt and Nr, to facilitate computation later
H_list = np.transpose(H_list, (0, 2, 3, 1, 4))
io.savemat('./data/channel_current.mat',{'H_list':H_list})
print(H_list.shape)  # (data_num,Nr,Nt,num_sc,2)
